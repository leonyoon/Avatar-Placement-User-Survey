...
public class Furnitures : MonoBehaviour
{
    ...
    {
        float[] S2_cat_freq_ava = new float[46];
        Ava_arrow_x = Mathf.Cos((90f - temp_AvaX.transform.eulerAngles.y) / 360f * (2f * Mathf.PI)) + temp_AvaX.transform.position.x;
        Ava_arrow_z = Mathf.Sin((90f - temp_AvaX.transform.eulerAngles.y) / 360f * (2f * Mathf.PI)) + temp_AvaX.transform.position.z;
        Ava_front = new Vector3(Ava_arrow_x - temp_AvaX.transform.position.x, 0f, Ava_arrow_z - temp_AvaX.transform.position.z);
        ...
        for (int i = 0; i < S2_Furnit_list.Length; i++)
        {
            ...
            if (vec_dist.magnitude < 3f)
            {
                //Debug.Log(S2_Furnit_list[i].Category);
                S2_fur_list.Add(S2_Furnit_list[i].Category);

                //behavior
                if (IndexOfCategory(S2_Furnit_list[i].Category) == 0)
                { 
                    S2_cat_freq_ava[0] += 0.1f; S2_cat_freq_ava[4] += 0.1f; S2_cat_freq_ava[7] += 0.1f; S2_cat_freq_ava[9] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 1)
                {
                    S2_cat_freq_ava[2] += 0.1f; S2_cat_freq_ava[3] += 0.1f; S2_cat_freq_ava[5] += 0.1f; S2_cat_freq_ava[6] += 0.1f; S2_cat_freq_ava[7] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 2)
                {
                    S2_cat_freq_ava[0] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 3)
                {
                    S2_cat_freq_ava[1] += 0.1f; S2_cat_freq_ava[6] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 4)
                {
                   S2_cat_freq_ava[6] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 5)
                {
                    S2_cat_freq_ava[6] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 6)
                {
                    S2_cat_freq_ava[8] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 9)
                {
                    S2_cat_freq_ava[6] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 10)
                {
                    S2_cat_freq_ava[6] += 0.1f;
                }
                else if (IndexOfCategory(S2_Furnit_list[i].Category) == 11)
                {
                    S2_cat_freq_ava[1] += 0.1f; S2_cat_freq_ava[6] += 0.1f;
                }

                //num
                S2_cat_freq_ava[10 + IndexOfCategory(S2_Furnit_list[i].Category)] += 1/ 10f; ;
                //dist
                S2_cat_freq_ava[22 + IndexOfCategory(S2_Furnit_list[i].Category)] += (3f - vec_dist.magnitude) / 3f/5f;
                
                float fur_ang = ContAngle(temp_AvaX.transform.forward, S2_Furnit_list[i].Position - temp_AvaX.transform.position);
                fur_ang = Mathf.Min(fur_ang, (360f - fur_ang));
                S2_cat_freq_ava[34 + IndexOfCategory(S2_Furnit_list[i].Category)] += (180f - fur_ang) / 180f / 5f;
    ...
    public float[] Object_category_frequency_HumX(GameObject temp_HumX)
    {
        ...
        float[] S1_cat_freq_hum = new float[46];
        ...
        for (int i = 0; i < S1_Furnit_list.Length; i++)
        {
            ...
            if (vec_dist.magnitude < 3f)
            {
                //Debug.Log(S1_Furnit_list[i].Category);
                //Debug.Log("Map1_Fur[" + i + "].Category : " + Map1_Fur[i].Category); // + "                    Dist : " + vec_dist.magnitude);
                S1_fur_list.Add(S1_Furnit_list[i].Category);
                ...
                //behavior
                if (IndexOfCategory(S1_Furnit_list[i].Category) == 0)
                {
                    S1_cat_freq_hum[0] += 0.1f; S1_cat_freq_hum[4] += 0.1f; S1_cat_freq_hum[7] += 0.1f; S1_cat_freq_hum[9] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 1)
                {
                    S1_cat_freq_hum[2] += 0.1f; S1_cat_freq_hum[3] += 0.1f; S1_cat_freq_hum[5] += 0.1f; S1_cat_freq_hum[6] += 0.1f; S1_cat_freq_hum[7] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 2)
                {
                    S1_cat_freq_hum[0] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 3)
                {
                    S1_cat_freq_hum[1] += 0.1f; S1_cat_freq_hum[6] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 4)
                {
                    S1_cat_freq_hum[6] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 5)
                {
                    S1_cat_freq_hum[6] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 6)
                {
                    S1_cat_freq_hum[8] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 9)
                {
                    S1_cat_freq_hum[6] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 10)
                {
                    S1_cat_freq_hum[6] += 0.1f;
                }
                else if (IndexOfCategory(S1_Furnit_list[i].Category) == 11)
                {
                    S1_cat_freq_hum[1] += 0.1f; S1_cat_freq_hum[6] += 0.1f;
                }

                //num
                S1_cat_freq_hum[10 + IndexOfCategory(S1_Furnit_list[i].Category)]+= 1/10f;
                //dist
                S1_cat_freq_hum[22 + IndexOfCategory(S1_Furnit_list[i].Category)] += (3f - vec_dist.magnitude) / 3f / 5f;

                float fur_ang = ContAngle(temp_HumX.transform.forward, S1_Furnit_list[i].Position - temp_HumX.transform.position);
                fur_ang = Mathf.Min(fur_ang, (360f - fur_ang));
                S1_cat_freq_hum[34 + IndexOfCategory(S1_Furnit_list[i].Category)] += (180f - fur_ang) / 180f / 5f;
