...
public class Pathfinder : MonoBehaviour 
{
    ...
    //Going public to access from Experiment.cs*
    ...
    public int startX, startZ, endX, endZ, width, height, sector;
	...
	void Start () 
    {
        ...
        startX = (int)MapStartPosition.x;
        startZ = (int)MapStartPosition.y;
        endX = (int)MapEndPosition.x;
        endZ = (int)MapEndPosition.y;

        //Find tile width and height
        width = (int)((endX - startX) / Tilesize);
        height = (int)((endZ - startZ) / Tilesize);
        */

        /*
        Map1 = Map;
        UnityDebug.Log("Map1[0,0] : [ " + Map1[0, 0].xCoord + " , " + Map1[0, 0].zCoord + " ] ");
        Debug.Log("Map1[width-1,0] : [ " + Map1[width - 1, 0].xCoord + " , " + Map1[width - 1, 0].zCoord + " ] ");
        Debug.DrawLine(new Vector3(Map1[0, 0].xCoord, 0, Map1[0, 0].zCoord), new Vector3(Map1[width - 1, 0].xCoord, 0, Map1[width - 1, 0].zCoord), Color.red);
        */
    }
    ...
    public void CreateMap()
    {
        ...
        */
        ...
        //Set map up
        Map = new Node[width, height, sector];
        float[,,] similarity = new float[width, height, sector];
        int size = width* height*sector;
        int levelOfAff = 1; int levelOfSpa = 6;
        SetListsSize(size);
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                ...
                {
                    float[] feature1 = new float[levelOfAff * sector];
                    float[] feature2 = new float[levelOfSpa * sector * 7];
                    float[] feature3 = new float[13];
                    ...
                    foreach (RaycastHit h in hit)
                    {
                        if (DisallowedTags.Contains(h.transform.tag))
                        {
                            if (h.point.y > maxY)
                            {
                                ...
                                Map[j, i, k] = new Node(j, i, k, 0, ID, x, y, false, feature1, feature2, feature3, similarity); //Non walkable tile!
                                free = false;
                                maxY = h.point.y;
                            }
                        }
                        ...
                        else
                        {
                            if (h.point.y > maxY)
                            {
                                ...
                                Map[j, i, k] = new Node(j, i, k, h.point.y, ID, x, y, true, feature1, feature2, feature3, similarity); //walkable tile!
                                free = false;
                                maxY = h.point.y;
                            }
                        }
    ...
    {      
        ...
        if (n.walkable)
        {
            return new Node(x, z, 0, n.yCoord, n.ID, n.xCoord, n.zCoord, n.walkable);
        }
        else
        {
            ...
            for (int i = z - 1; i < z + 2; i++)
            {
                for (int j = x - 1; j < x + 2; j++)
                {
                    ...
                    {
                        if (Map[j, i, 0].walkable)
                        {
                            return new Node(j, i, 0, Map[j, i, 0].yCoord, Map[j, i, 0].ID, Map[j, i, 0].xCoord, Map[j, i, 0].zCoord, Map[j, i, 0].walkable);
                        }
                    }
                }
            }
    ...
    {       
        ...
        if (walkableNodes.Count > 0) //If we found some walkable tiles we will then return the nearest
        {
            ...
            endNode = new Node(n.x, n.y, 0, n.yCoord, n.ID, n.xCoord, n.zCoord, n.walkable);
        }
    }
                ...
                if (i > -1 && i < Map.GetLength(1) && j > -1 && j < Map.GetLength(0))
                {
                    ...
                    if (i != y || j != x)
                    {
                        ...
                        if (Map[j, i, 0].walkable)
                        {
                            ...
                            if (!OnClosedList(Map[j, i, 0].ID))
                            {
                                //Check if we can move up or jump down!
                                if ((Map[j, i, 0].yCoord - currentNode.yCoord < ClimbLimit && Map[j, i, 0].yCoord - currentNode.yCoord >= 0) || (currentNode.yCoord - Map[j, i, 0].yCoord < MaxFalldownHeight && currentNode.yCoord >= Map[j, i, 0].yCoord))
                                {
                                    //If it is not on the open list then add it to
                                    if (!OnOpenList(Map[j, i, 0].ID))
                                    {
                                        Node addNode = new Node(Map[j, i, 0].x, Map[j, i, 0].y, 0, Map[j, i, 0].yCoord, Map[j, i, 0].ID, Map[j, i, 0].xCoord, Map[j, i, 0].zCoord, Map[j, i, 0].walkable, Map[j, i, 0].feature1, Map[j, i, 0].feature2, Map[j, i, 0].feature3, Map[j, i, 0].similarity, currentNode);
                                        addNode.H = GetHeuristics(Map[j, i, 0].x, Map[j, i, 0].y);
                                    ...
                                    else
                                    {
                                       ///If it is on openlist then check if the new paths movement cost is lower
                                        Node n = GetNodeFromOpenList(Map[j, i, 0].ID);
                                        if (currentNode.G + GetMovementCost(x, y, j, i) < openList[Map[j, i, 0].ID].G)
                                        {
    ...
    void OnDrawGizmosSelected()
    {
         ...
         //           {
         //               Gizmos.color = (Map[j, i, 0].walkable) ? new Color(0, 0.8F, 0, 0.5F) : new Color(0.8F, 0, 0, 0.5F);
         //               Gizmos.DrawCube(new Vector3(Map[j, i, 0].xCoord, Map[j, i, 0].yCoord + 0.1F, Map[j, i, 0].zCoord), new Vector3(Tilesize, 0.5F, Tilesize));
         //           }
         //       }
         //   }
    }
                ...
                for (int j = 0; j < Map.GetLength(0); j++)
                {
                    ...
                    for (int y = i - 1; y < i + 2; y++)
                    {
                        for (int x = j - 1; x < j + 2; x++)
                        {
                            ...
                            if (Map[j, i, 0].yCoord > Map[x, y, 0].yCoord && Mathf.Abs(Map[j, i, 0].yCoord - Map[x, y, 0].yCoord) > MaxFalldownHeight)
                                continue;

                            if (Map[j, i, 0].yCoord < Map[x, y, 0].yCoord && Mathf.Abs(Map[x, y, 0].yCoord - Map[j, i, 0].yCoord) > ClimbLimit)
                                continue;
                          
                            Vector3 start = new Vector3(Map[j, i, 0].xCoord, Map[j, i, 0].yCoord + 0.1f, Map[j, i, 0].zCoord);
                            Vector3 end = new Vector3(Map[x, y, 0].xCoord, Map[x, y, 0].yCoord + 0.1f, Map[x, y, 0].zCoord);
    ...
    public void DynamicRedoMapEdit(List<Vector2> ids)
    {
        ...
        {
            Map[(int)v.x, (int)v.y, 0].walkable = true;
        }
    }
        ...
        for (int i = startNode.x - 2; i < startNode.x + xIterations; i++)
        {
            for (int j = startNode.y - 2; j < startNode.y + zIterations; j++)
            {
                if (i >= 0 &&  j >= 0 && i < Map.GetLength(0) && j < Map.GetLength(1))
                {
                    ...
                    if (CheckFullTileSize)
                    {
                        hit = Physics.SphereCastAll(new Vector3(Map[i, j, 0].xCoord, HighestPoint, Map[i, j, 0].zCoord), Tilesize / 2, Vector3.down, dist);
                    }
                    else
                    {
                        hit = Physics.SphereCastAll(new Vector3(Map[i, j, 0].xCoord, HighestPoint, Map[i, j, 0].zCoord), Tilesize / 16, Vector3.down, dist);
                    }
                    ...
                    foreach (RaycastHit h in hit)
                    {
                        if (DisallowedTags.Contains(h.transform.tag))
                        {
                            ...
                            {
                                //It is a disallowed walking tile, make it false
                                Map[i, j, 0].walkable = false;
                                Map[i, j, 0].yCoord = h.point.y;
                                maxY = h.point.y;
                            }
                        }
                        else if (IgnoreTags.Contains(h.transform.tag))
    ...
    {      
        ...
        foreach (Vector3 pos in vList)
        {
            ...
            {
                if (Map[x, z, 0].walkable)
                {
                    Map[x, z, 0].walkable = false;
